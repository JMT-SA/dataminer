...
class TestSecurityRoutes < RouteTester
  ...
  def test_delete
    ...
    delete 'security/functional_areas/functional_areas/1', {}, 'rack.session' => { user_id: 1, last_grid_url: '/dataminer/reports' }
    expect_ok_redirect url: '/dataminer/reports'
  end

  def test_delete_fail
    SecurityApp::FunctionalAreaInteractor.any_instance.stubs(:delete_functional_area).returns(bad_response)
    delete 'security/functional_areas/functional_areas/1', {}, 'rack.session' => { user_id: 1, last_grid_url: '/dataminer/reports' }
    expect_bad_redirect url: '/dataminer/reports'
  end
  ...
  def test_create
    ...
    post 'security/functional_areas/functional_areas', {}, 'rack.session' => { user_id: 1, last_grid_url: '/dataminer/reports' }
    expect_ok_redirect url: '/dataminer/reports'
  end
